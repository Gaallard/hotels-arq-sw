...
export async function login(userData) {
  try {
        const response = await axios.post('http://localhost:8081/users/login', userData, {
            credentials: "include",
        });
...
export async function insertHotel(Data){
  try {
        const response = await axios.post('http://localhost:8080/hotels', Data, {
...
export async function updateHotel(hotelId, Data) {
    try {
        const response = await axios.put(`http://localhost:8080/hotels/${hotelId}`, Data, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
...
export async function reserva(Data){
  ...
  return axios.post('http://localhost:8082/reservas',data, {
    headers: { 'Authorization': `Bearer ${authToken}` }
  })
    .then(response => {
...
export async function updateReserva(Data){
    ...
    console.log("data id: ",data)
    return axios.put('http://localhost:8082/reservas',data, {
      headers: { 'Authorization': `Bearer ${authToken}` }
    })
      .then(response => {
        console.log('Reserva realizada: ', response)
  ...
  export async function deleteReserva(Data){
    ...
    console.log("data id: ",data)
    return axios.delete('http://localhost:8082/reservas',data, {
      headers: { 'Authorization': `Bearer ${authToken}` }
    })
      .then(response => {
        console.log('Reserva realizada: ', response)
...
export async function tokenId(){
    ...
    const val1 = await axios.get('http://localhost:8081/users/token', {
    headers: {
      'Authorization': token
    }
  });

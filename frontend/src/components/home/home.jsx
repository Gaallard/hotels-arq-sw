...
/*
  const handleCantidadNoches = async () => {
    try {
      const searchResults = await reserva(noches); // Puedes ajustar offset y limit según necesites
      setHotels(searchResults.results || searchResults);
      setMensaje("Resultados para", `${searchQuery}`);
    } catch (error) {
      console.error("Error al buscar hoteles:", error);
...
const MisHoteles = () => {
  ...
  const [reservas, setReservas] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const [isReserved, setIsReserved] = useState({}); // Objeto para el estado de reserva
...
const handleReserva = (hotelId) => {
  setIsReserved((prevState) => ({ ...prevState, [hotelId]: true })); // Actualiza solo el frontend
};

const handleSearch = async () => {
  try {
    const searchResults = await search(searchQuery, 0, 10);
    setHotels(searchResults.results || searchResults);
    setMensaje(`Resultados para: "${searchQuery}"`);
  } catch (error) {
    console.error("Error al buscar hoteles:", error);
  ...
  {hotels.length > 0 ? (
    hotels.map((data) => (
      <li key={data.id} className="bloque">
        ...
        <div className="boton-reserva">
          <button onClick={() => handleReserva(data._id)}>
            {isReserved[data._id] ? 'Reservado' : 'Reservar'}
          </button>
        </div>
        {isAdmin && (
          <button className="boton-editar" onClick={() => openEditDialog(data)}>
